...
# ---------------------------------------------------------------------------
# B-pre-3 regression coverage: Qdrant orphan-point prevention
# ---------------------------------------------------------------------------


def make_chunk(chunk_id: str, collection_name: str) -> Chunk:
    """Build a Chunk suitable for the Qdrant push helpers."""
    return Chunk(
        id=chunk_id,
        content=f"content for {chunk_id}",
        embedding_model="test-model",
        vector=[0.1] * 1024,
        item_metadata={"tags": ["test"]},
        collection_name=collection_name,
    )


def test_push_chunks_to_qdrant_partial_failure_rolls_back():
    """If a multi-collection upsert fails partway, the successful collection
    upserts are rolled back so the caller sees all-or-nothing.

    Without this rollback, B-pre-3 would leave orphan points in the first
    collection while the SQL chunks rows referenced ALL collections — search
    would return some chunks but not others depending on collection.
    """
    chunks = [
        make_chunk("00000000-0000-0000-0000-000000000001", "mail"),
        make_chunk("00000000-0000-0000-0000-000000000002", "mixed"),
    ]

    # Succeed on the first call, fail on the second.
    upsert = MagicMock(side_effect=[None, RuntimeError("qdrant boom")])
    delete = MagicMock()

    with (
        patch("memory.common.qdrant.upsert_vectors", upsert),
        patch("memory.common.qdrant.delete_points", delete),
        patch("memory.common.qdrant.get_qdrant_client"),
    ):
        with pytest.raises(RuntimeError, match="qdrant boom"):
            push_chunks_to_qdrant(chunks)

    assert upsert.call_count == 2
    # Successful collection's points must be deleted.
    delete.assert_called_once()
    deleted_collection = delete.call_args.args[1]
    deleted_ids = delete.call_args.args[2]
    assert deleted_collection in {"mail", "mixed"}
    assert len(deleted_ids) == 1


def test_push_chunks_to_qdrant_first_collection_failure_no_rollback_needed():
    """When the very first upsert fails, nothing was written, so no delete."""
    chunks = [make_chunk("00000000-0000-0000-0000-000000000003", "mail")]

    upsert = MagicMock(side_effect=RuntimeError("qdrant down"))
    delete = MagicMock()
...
def test_rollback_qdrant_writes_swallows_delete_errors(caplog):
    """The rollback path must NEVER mask the original error — even if the
    cleanup itself fails, we log and return cleanly."""
    written = [("mail", ["00000000-0000-0000-0000-000000000004"])]
    client = MagicMock()
    delete = MagicMock(side_effect=IOError("qdrant unreachable"))

    with patch("memory.common.qdrant.delete_points", delete):
        # Must not raise.
...
def test_rollback_qdrant_writes_skips_empty_collections():
    """An empty id-list for a collection is a no-op (defensive)."""
    delete = MagicMock()
    with patch("memory.common.qdrant.delete_points", delete):
        rollback_qdrant_writes(MagicMock(), [("mail", [])])
    delete.assert_not_called()


def test_process_content_item_orphan_cleanup_on_final_commit_failure(
    db_session, qdrant, mock_uuid4
):
    """If the final embed_status commit fails after Qdrant was written,
    the just-written Qdrant points must be deleted so re-runs can re-index
    cleanly. This is the core B-pre-3 hardening.
    """
    mail_message = MailMessage(
        sha256=b"orphan_test_hash" + bytes(16),
        tags=["test"],
        size=100,
        mime_type="message/rfc822",
        embed_status="RAW",
        message_id="<orphan-test@example.com>",
        subject="Orphan test",
        sender="sender@example.com",
        recipients=["recipient@example.com"],
        content="Orphan prevention content",
        folder="INBOX",
        modality="mail",
    )
    ...
    real_chunk = Chunk(
        ...
        content="orphan prevention chunk",
        embedding_model="test-model",
        vector=[0.1] * 1024,
        item_metadata={"tags": ["test"]},
        collection_name="mail",
    ...
    # Patch only the SECOND .commit() — the one that persists embed_status —
    # to fail. The earlier commit that writes the chunks must succeed so the
    # Qdrant write actually happens.
    ...
    def commit_side_effect():
        ...
        if commit_calls["n"] == 1:
            return real_commit()
        raise RuntimeError("simulated post-qdrant DB failure")

    with (
        patch("memory.common.embedding.embed_source_item", return_value=[real_chunk]),
        patch(
            "memory.common.content_processing.push_chunks_to_qdrant"
        ) as mock_push,
        patch("memory.common.qdrant.delete_points", delete),
        patch.object(db_session, "commit", side_effect=commit_side_effect),
    ):
        mock_push.return_value = None  # successful Qdrant write
        with pytest.raises(RuntimeError, match="simulated post-qdrant DB failure"):
            process_content_item(mail_message, db_session)

    # Confirm we reached the post-Qdrant-write commit.
    assert commit_calls["n"] == 2
    mock_push.assert_called_once()
    # Qdrant cleanup must have been triggered.
    delete.assert_called_once()
    assert delete.call_args.args[1] == "mail"
    assert delete.call_args.args[2] == [str(real_chunk.id)]


def test_process_content_item_no_cleanup_when_qdrant_was_not_written(
    db_session, qdrant, mock_uuid4
):
    """If the Qdrant write itself failed, there's nothing to roll back —
    the existing failure path should not double-fault on a phantom delete."""
    mail_message = MailMessage(
        sha256=b"no_qdrant_hash" + bytes(18),
        tags=["test"],
        size=100,
        mime_type="message/rfc822",
        embed_status="RAW",
        message_id="<no-qdrant@example.com>",
        subject="No qdrant",
        sender="sender@example.com",
        recipients=["recipient@example.com"],
        content="No qdrant test",
        folder="INBOX",
        modality="mail",
    )
    ...
    with (
        ...
        patch(
            "memory.common.content_processing.push_chunks_to_qdrant",
            side_effect=RuntimeError("qdrant push failed"),
        ),
        patch("memory.common.qdrant.delete_points", delete),
    ):
        # The function catches the Qdrant push error and sets FAILED, but does
        # NOT re-raise — so no exception bubbles. The point is: delete_points
        # must NOT have been called via the post-commit cleanup path, because
        # qdrant_written stayed False.
        result = process_content_item(mail_message, db_session)

    assert result["status"] == "failed"
    assert result["embed_status"] == "FAILED"
    delete.assert_not_called()
