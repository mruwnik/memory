...
#
# The hex suffix is generated by ``cloud_claude.spawn_session_id`` via
# ``secrets.token_hex(6)`` -> 12 hex chars. The canonical session-id regex
# lives in ``cloud_claude._SESSION_ID_RE``; if those token-hex semantics
# ever change, both regexes must stay in sync.
#
...
#
# 2. **Hex length floor** (``{12,}``). Without it, ``[a-fA-F0-9]+`` accepts
...
_CLAUDE_SESSION_PATTERN = re.compile(
    r"^/claude/u\d+-(?:e\d+|s\d+|x)-[a-fA-F0-9]{12,}(?:/|$)",
    re.IGNORECASE,
)
