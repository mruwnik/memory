...
class GoogleOAuthState:
    ...
    @classmethod
    def create(cls, user_id: int, code_verifier: str | None = None) -> str:
        state = secrets.token_urlsafe(32)
        key = f"{cls.KEY_PREFIX}{state}"
        # The PKCE code_verifier must survive between the /authorize and
        # /callback requests (each builds its own Flow), so it rides along
        # with the user id in the state payload.
        payload = json.dumps({"user_id": user_id, "code_verifier": code_verifier})
        cls._get_redis().setex(key, cls.STATE_TTL_SECONDS, payload)
        return state

    @classmethod
    def validate(cls, state: str) -> tuple[int, str | None] | None:
        key = f"{cls.KEY_PREFIX}{state}"
...
def google_authorize(
    ...
    # Mint the PKCE verifier here rather than letting the Flow autogenerate
    # one: the callback runs in a separate request with its own Flow, so the
    # verifier must round-trip through the state storage (43-128 chars per
    # RFC 7636; token_urlsafe(96) yields 128).
    ...
    state = GoogleOAuthState.create(user.id, code_verifier)
...
def google_callback(
    ...
    # Validate state
    validated = GoogleOAuthState.validate(state)
    if not validated:
        return HTMLResponse(
            content="<html><body><h1>Invalid or Expired State</h1></body></html>",
            status_code=400,
        )
    user_id, code_verifier = validated
    ...
    with make_session() as session:
        ...
        flow = Flow.from_client_config(
            ...
            redirect_uri=redirect_uri,
            # Echo the verifier whose challenge went out with /authorize;
            # autogenerating a fresh one here would fail the PKCE check.
            code_verifier=code_verifier,
...
def reauthorize_account(
    ...
    # See google_authorize for why the verifier is minted here and stored
    # in the state rather than autogenerated by the Flow.
    code_verifier = secrets.token_urlsafe(96)
    flow = Flow.from_client_config(
        oauth_config.to_client_config(),
        scopes=requested_scopes,
