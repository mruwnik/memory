...
- **Scopes**: keys inherit user scopes unless overridden; one-time keys are revoked on first use (minted only via the MCP `create_one_time_key` tool, TTL'd and rate-limited per user; a maintenance task purges consumed/expired ones)
